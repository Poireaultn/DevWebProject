        ...
        <div class="col-md-4 mb-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('cameras.toggle', $camera) }}" method="POST" class="mb-2">
                        @csrf
                        <button type="submit" class="btn {{ $camera->is_on ? 'btn-danger' : 'btn-success' }} w-100">
                            {{ $camera->is_on ? 'Éteindre' : 'Allumer' }}
                        </button>
                    </form>
                </div>
            </div>
